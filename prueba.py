...
def polynomial(d):  # pasando cadena de numeros(indices del polinomio)---> d="410"
...
def CRC(trama, pol):
    ...
    datoFinal = modDivFunc(adjunto, polinomio)
    return datoFinal


'''
    # preparamos la trama con el residuo en funcion del polinomio
    residuo = ""
    bits = len(pol)
    # tanats iteraciones como bits
    for n in range(bits-1):
        residuo += "0"
    # anadirlo al final ala trama
    trama_completa = trama+residuo
    i = 0
    # trama/div
    encriptado = xorFunc(trama_completa, pol)
